...
class BaseEnvLoop(ABC):
    ...
    @abstractmethod
    def rollout(
        self, loop_state: Union[EnvLoopState, None], params: Any, apply_fn: Callable, steps_per_env: int, progress_bar: bool
    ) -> Tuple[DefaultTimeStep, EnvLoopState]:
        raise NotImplementedError("Rollout not defined")

    @abstractmethod
    def reset_loop(self) -> EnvLoopState:
        """
...
####Change apply_fn to a model instead later!
    def rollout(
        self,
        loop_state: Union[EnvLoopState, None],
        apply_fn: nn.Module ,#this will be actually be the actor model!
        steps_per_env: int,
        ...
        for t in p_bar:
            ...
            #print(f"roll out actions : {actions}")
            (
                next_observations,
                rewards,
                terminations,
            ...
            if self.is_torch_gpu_env:#fix this maybe?
            ...
            if "final_info" in infos:
               # data["final_info"] = tools.to_numpy(infos["final_info"])
                data["final_info"] = infos["final_info"]
            for k, v in rb.items():
                data[k].append(v)
            ...
            ep_lengths[dones] = 0
        ############################################
        #Check if the output data dict is correct!##
        ############################################
        # stack data
        for k in data:
